...
Room.prototype.updateConstructionSites = function() {
	...
	*/
	...
	if(this.find(FIND_MY_CONSTRUCTION_SITES).length == 0 && Game.time % 2 == 0) //Every other tick so we avoid double building
	{
		...
		convertConstructionFlags(this);
	}
};

//Flags look like this: !CONSTRUCT|BUILDINGTYPE|UNIQUEIFIER

function convertConstructionFlags(room)
{
	var constructionFlagArray = lookForConstructionFlags(room);
...
function lookForConstructionFlags(room)
{
	var flagArray = [];
	var flagsInRoom = room.find(FIND_FLAGS);

	for(var i = 0; i < flagsInRoom.length; i++)
	{
		var flagName = flagsInRoom[i].name;
		if(flagName.startsWith('!CONSTRUCT|'))
		{
			flagArray.push(flagsInRoom[i]);
		}
	}
...
Room.prototype.convertPlanFlagToConstruction = function() {
	var planFlags = lookForPlanFlags(this);

	var fullStructTypes = [];

	var convertedFlag = false;

	console.log(planFlags.length);
	...
	for(i = 0; i < planFlags.length; i++)
	{
		...
		if(this.hasNoBuildingSlots(structureType) && !fullStructTypes.includes(structureType)) {
			console.log('WARNING: Room ' + this.name + ' cannot build ' + structureType);
			fullStructTypes.push(structureType);
		}
		
		else {
			console.log('Room ' + this.name + ' placing flag to build ' + structureType);
			...
			if(placingObstacleStructure) {
				...
				if(invalidStructureUnderneath !== undefined) {
					if(invalidStructureUnderneath.my) {
						console.log('DESTROYING INVALID ' + invalidStructureUnderneath.structureType + ' at ' + invalidStructureUnderneath.pos.x + 'x ' + invalidStructureUnderneath.pos.y + 'y ' + this.name);
						...
						console.log('WARNING: Room ' + this.name + ' cannot build ' + planFlag.name + ' because something is in the way');
						continue;
					}
				}
			}
...
function lookForPlanFlags(room)
{
	var flagArray = room.find(FIND_FLAGS);
	var planFlags = []
	...
	for(var i = 0; i < flagArray.length; i++)
	{
		...
		if(flag.name.startsWith('!PLAN|'))
		{
			planFlags.push(Game.flags[flag.name]);
		}
	}
...
var chunkDefinitions = {
	"heart": {
		...
		"3": {
			"buildings":{"storage":{"pos":[]},"spawn":{"pos":[{"x":25,"y":23}]},"extension":{"pos":[{"x":27,"y":23},{"x":26,"y":24},{"x":26,"y":25},{"x":28,"y":23},{"x":28,"y":24},{"x":27,"y":25},{"x":29,"y":24},{"x":29,"y":25},{"x":28,"y":26},{"x":30,"y":25}]},"link":{"pos":[]},"road":{"pos":[{"x":29,"y":26},{"x":28,"y":25},{"x":27,"y":24},{"x":26,"y":23},{"x":25,"y":22},{"x":24,"y":22},{"x":25,"y":21},{"x":24,"y":23},{"x":24,"y":24},{"x":24,"y":25},{"x":24,"y":26},{"x":26,"y":21}]},"terminal":{"pos":[]},"tower":{"pos":[{"x":25,"y":26}]}}
		},
		"2": {
			"buildings":{"storage":{"pos":[]},"spawn":{"pos":[{"x":25,"y":23}]},"extension":{"pos":[{"x":27,"y":23},{"x":26,"y":24},{"x":26,"y":25},{"x":28,"y":23},{"x":28,"y":24}]},"link":{"pos":[]},"road":{"pos":[{"x":27,"y":24},{"x":26,"y":23},{"x":25,"y":22},{"x":25,"y":21},{"x":26,"y":21}]},"terminal":{"pos":[]},"tower":{"pos":[]}}
		},
		"1": {
			"buildings":{"storage":{"pos":[]},"spawn":{"pos":[{"x":25,"y":23}]},"extension":{"pos":[]},"link":{"pos":[]},"road":{"pos":[]},"terminal":{"pos":[]},"tower":{"pos":[]}}
		},
	...
	"sword": {
		...
		"3": {
			...
			"buildings":{
				...
				"road":{"pos":[{"x":26,"y":22},{"x":25,"y":21},{"x":24,"y":20},{"x":24,"y":19},{"x":23,"y":20},{"x":23,"y":21},{"x":23,"y":22},{"x":23,"y":23},{"x":25,"y":19},{"x":26,"y":19},{"x":27,"y":19},{"x":27,"y":23}]},
				"tower":{"pos":[{"x":25,"y":17}]}
			}
		},
		"2": {
		...
		"1": {
			...
			"buildings":{
				"spawn":{"pos":[{"x":24,"y":21}]},
				"road":{"pos":[{"x":25,"y":21},{"x":24,"y":20},{"x":24,"y":19},{"x":23,"y":20},{"x":23,"y":21},{"x":23,"y":22},{"x":25,"y":19}]}
			}
		}
	},
	"bigbattery56": {
